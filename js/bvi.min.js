(function ($) {
    ...
    $.bvi = function (options) {
        ...
        function submitCheck () {
            var tes = localStorage.formSubmitCheck;
            var trueString = "true";
            if (tes == trueString) {
                var messege = $("#registration_email + .invalid-feedback");
                bvi_tts_speak(messege.text());
                bvi_tts_player();
                localStorage.formSubmitCheck = false;
            }

            $('form[name="registration"]').on("submit", function () {
                    localStorage.formSubmitCheck = true;
            });
        }
        submitCheck ();

        function selectRework () {
            $("select").focusin(function(){
                if ($("div").is(".bvi-body") ) {
                    var n = $(this).children("option").length;
                    $(this).attr("size", n).css(
                        "max-height", "200px"
                    );
                }
            });
            ...
            $("select").on('mouseenter','option',function(e) {
                var $target = $(e.target);
                $target.css(
                    "background-color", "rgba(0,0,0,.2) !importatnt"
                );
                bvi_tts_speak($target.text());
                bvi_tts_player();
            });
        ...
        function addTtsClasses () {
            $(".bvi-open").on("click", function () {
                console.log("open");
                var textContentMassive = ["h1","h2","h3","h4","h5","p","span:not(.required-mark)","a:not(.content):not(#scrollUp):not(.logo)",".text-items_list li","button"];
                for (let textElement of textContentMassive) {
                    $(textElement).addClass("bvi-tts");
                }
        ...
        function bvi_tts_player() {
            ...
            $('form[name="registration"] .btn').remove();
            $('form[name="registration"]').append('<button class="btn btn-md btn-primary validation_button" type="submit">Зарегистрироваться </button>');
            ...
            if (Cookies.get('bvi-tts') === 'true' && bvi_tts_support_browser) {
                ...
                $('.bvi-tts-text').mouseenter(function () {
                    bvi_tts_synth.cancel();
                    var bvi_tts_class = $(this).siblings().data('bvi-tts-class-text');
                    var bvi_tts_links_id = $(this).siblings().data('bvi-tts-link-id');
                ...
                $('input').change(function () {
                    var innerText = this;
                    var curLabel = innerText.closest(".form-group").querySelector("label").firstChild.data;
                    bvi_tts_speak("Поле  -" + curLabel+ "изменено на " + innerText.value);
                    return false;
                });
                ...
                $('input').focus(function () {
                    var innerText = this;
                    var curLabel = innerText.closest(".form-group").querySelector("label").firstChild.data;
                    bvi_tts_speak("Поле  -" + curLabel+ "с данными: " + innerText.value);
                    return false;
                });

                $('input').keydown(function (event) {
                    bvi_tts_speak(this.value);
                    bvi_tts_speak(event.key);
                });
                ...
                $('input.phone').focus(function () {
                    ...
                    var curLabel = innerText.closest(".form-group").querySelector("label").firstChild.data;
                    if (innerText.value === "") {
                        bvi_tts_speak("Поле  -" + curLabel+ "с данными: +7");
                    }
                    else {
                        bvi_tts_speak("Поле  -" + curLabel+ "с данными: " + innerText.value);
                    }
                ...
                $('input[type="date"]').focus(function () {
                    ...
                    var dmg = birthday.split("-");
                    var d = new Date(dmg[0], dmg[1]-1, dmg[2]);
                    let correctData = d;
                    bvi_tts_speak("Дата рождения - " + correctData.toLocaleString());
                    return false;
                });
                ...
                $('input[type="date"]').change(function () {
                    ...
                    var d = new Date(dmg[0], dmg[1]-1, dmg[2]);
                    let correctData = d;
                    bvi_tts_speak("дата рождения измена на - " + correctData.toLocaleString());
                    return false;
                });

                $('.validation_button').click(function (e) {
                    var requiredMark = $("form > .form-group label span");
                    for (let i = 0; i < requiredMark.length; i++) {
                        if ($(requiredMark[i]).is(".required-mark")) {
                            if ($(requiredMark[i].closest(".form-group").childNodes).is("input")) {
                                var markeredInput = requiredMark[i].closest(".form-group").querySelector("input").value;
                            }
                            if ($(requiredMark[i].closest(".form-group").childNodes).is("select")) {
                                if (requiredMark[i].closest(".form-group").querySelector("select").value < 1) {
                                    var markeredInput = requiredMark[i].closest(".form-group").querySelector("select").value;
                                }
                            }
                            if ($(requiredMark[i].closest(".form-group").childNodes).is("input#registration_plainPassword_second")) {
                                var checkParol = requiredMark[i].closest(".form-group").querySelector("input").value,
                                    firstParol = requiredMark[i-1].closest(".form-group").querySelector("input").value;
                                if (checkParol != firstParol) {
                                    bvi_tts_speak("Пароли не совпадают");
                                }
                                else {
                                    // localStorage.formSubmitCheck = true;
                            ...
                            var markeredInputLabel = requiredMark[i].closest(".form-group").querySelector("label").firstChild.data;
                            if (markeredInput === "") {
                                var requiredText = markeredInputLabel
                                bvi_tts_speak("Поле  -" + requiredText+ " незаполнено");
                                break;
                            }
                        }
                    }
                ...
                $('select').change(function () {
                    var innerSelText = this;
                    var curLabel = innerSelText.closest(".form-group").querySelector("label").firstChild.data;
                    let selectorId =  innerSelText.getAttribute("id");
                    var txt = $("#"+selectorId+" option:selected").text();;
                    bvi_tts_speak("Поле  -" +curLabel+ "изменено на " + txt);
                    return false;
                });
                ...
                $('select').focus(function () {
                    ...
                    let selectorId =  innerSelText.getAttribute("id");
                    var txt = $("#"+selectorId+" option:selected").text();;
                    bvi_tts_speak("Поле  -" +curLabel+ "с данными: " + txt);
                    return false;
                });
        ...
        function bvi_click() {
            ...
            $("#bvi-tts-false").click(function () {
                ...
                // $('form[name="registration"] .btn').remove();
                // $('form[name="registration"]').append('<button class="btn btn-md btn-primary" type="submit">Зарегистрироваться </button>');
